...
func (self *Lexer) NextToken() Token {
	...
	case '<':
		...
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerUnexpectedLess(tk))
		return tk
	...
	case '#':
		...
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerUnexpectedSharp(tk))
		return tk
	...
	case '[':
		if self.attrDepth > 0 {
			self.report(digerrors.LexerNestedAttribute(self.tok(ILLEGAL)))
			self.attrDepth++
		}
	...
	default:
		...
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerUnknownCharacter(tk))
		return tk
	}
}
...
func (self *Lexer) multiLineComment() Token {
	...
	for {
		if self.rn == 0 {
			tk := self.tok(ILLEGAL)
			self.report(digerrors.LexerUnterminatedMultilineComment(tk))
			self.unfreeze()
			self.stabilize()
			return tk
		}
...
func (self *Lexer) readString() Token {
	...
	if self.rn == 0 {
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerUnterminatedString(tk))
		self.unfreeze()
		self.stabilize()
		return tk
	}
...
func (self *Lexer) readRawString() Token {
	...
	if self.rn == 0 {
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerUnterminatedRawString(tk))
		self.unfreeze()
		self.stabilize()
		return tk
	}
...
func (self *Lexer) readChar() Token {
	...
	if self.rn != '\'' {
		tk := self.tok(ILLEGAL)
		self.report(digerrors.LexerInvalidCharacterLiteral(tk))
		return tk
	}
	self.advance()
...
func (self *Lexer) report(err digerrors.Error) {
